...
<div class="container">
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-body">
            ...
            <p><strong>Tanggal:</strong> {{ $rekamMedis->tanggal ?? now()->format('Y-m-d') }}</p>
        </div>
    </div>
    ...
    <form action="{{ route('rekammedis.update', $rekamMedis->id) }}" method="POST">
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                <h5 class="fw-bold mb-3">Catatan Medis</h5>
                <div class="mb-3">
                    <label for="keluhan" class="form-label">Keluhan</label>
                    <textarea name="keluhan" id="keluhan" rows="3" class="form-control">{{ old('keluhan', $rekamMedis->keluhan) }}</textarea>
                </div>
                <div class="mb-3">
                    <label for="diagnosa" class="form-label">Diagnosa</label>
                    <textarea name="diagnosa" id="diagnosa" rows="3" class="form-control">{{ old('diagnosa', $rekamMedis->diagnosa) }}</textarea>
                </div>
                <div class="mb-3">
                    <label for="tindakan" class="form-label">Tindakan</label>
                    <textarea name="tindakan" id="tindakan" rows="3" class="form-control">{{ old('tindakan', $rekamMedis->tindakan) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <div class="d-flex justify-content-between align-items-center mt-4">
                    <!-- Tombol Kembali ke halaman detail (show) -->
                    <a href="{{ route('rekammedis.show', $rekamMedis->id) }}" class="btn btn-secondary">
                        ⬅️ Kembali
                    </a>
...
.legend {
    display:inline-block;width:18px;height:18px;border-radius:4px;margin-right:5px;
}
